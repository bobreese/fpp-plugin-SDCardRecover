#!/bin/bash
# Deep-scan fallback: raw signature-based carving for when the partition won't
# mount at all (even read-only) or sdcard_verify.sh found files the filesystem
# can no longer locate. Works directly against the block device, independent
# of the filesystem's health. Scoped to file types FPP actually uses so the
# user isn't handed thousands of irrelevant carved fragments.
#
